 ...
 */
import { msalConfigMicrosoftEntraClientID } from "./config/azureADMicrosoftEntraIDAuthConfig";
 ...
 */
export const msalInstanceMicrosoftEntraClientID = new PublicClientApplication(msalConfigMicrosoftEntraClientID);
...
(async () => {
    try {
        ...
        await msalInstanceMicrosoftEntraClientID.initialize();
         ...
         */
        if (!msalInstanceMicrosoftEntraClientID.getActiveAccount() && msalInstanceMicrosoftEntraClientID.getAllAccounts().length > 0) {
            msalInstanceMicrosoftEntraClientID.setActiveAccount(msalInstanceMicrosoftEntraClientID.getAllAccounts()[0]);
        }
         ...
         */
        msalInstanceMicrosoftEntraClientID.addEventCallback((event: EventMessage) => {
            if (event.eventType === EventType.LOGIN_SUCCESS && event.payload) {
                const payload = event.payload as AuthenticationResult;
                const account = payload.account;
                msalInstanceMicrosoftEntraClientID.setActiveAccount(account);
            }
        });
        ...
        root.render(
            <Router>
                ...
                <App pca={msalInstanceMicrosoftEntraClientID} />
            </Router>
        );
    } catch (error) {
        /**
